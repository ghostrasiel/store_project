...
def add_member(values):
    try:
        conn = pymysql.connect(**conninfo)
        cursor = conn.cursor()
        add_member="""insert into member_db(member_id , name , age , marital_status , income , hh_comp ,picture_array , picture_path ) 
                    values(%s , %s , %s ,%s,%s,%s,%s,%s);
                    """
        cursor.executemany(add_member , values)
        conn.commit()
        print('ok')
    except:
        print('異常')
...
def check_member():
    ...
    try:
        conn = pymysql.connect(**conninfo)
        cursor = conn.cursor()
        select_member="select id , member_id , name , picture_array , picture_path from member_db order by id"
...
def select_member(member_number):
    data = None
    try:
        conn = pymysql.connect(**conninfo)
        cursor = conn.cursor()
        select_member=f"select id , member_id , name  from member_db where id={member_number} order by id"
        cursor.execute(select_member)
        data = cursor.fetchall()
        print('ok')
    except:
...
def update_register(register_id , user):
    now = datetime.datetime.today()
    now = now.strftime('%Y-%m-%d %H:%M:%S')
    try:
        conn = pymysql.connect(**conninfo)
        cursor = conn.cursor()
        update = f"update cash_register set date='{now}' , user = '{user}' where id = '{register_id}' ; "
...
def check_register(register_id):
    data = None
    try:
        conn = pymysql.connect(**conninfo)
        cursor = conn.cursor()
...
def line_bot_push(barsket_data , user_id):
    ...
    line_bot_api.push_message(user_ID,
                              TextSendMessage(text=barsket_data))
...
if __name__ == '__main__':
    ...
    # if data[0][1] == 'None' :
    #     update_register(register_id='A' , user='uuuuuser-ikwerfwe-falksjkda')
    #     print('double ok')
